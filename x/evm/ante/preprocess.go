...
type EVMPreprocessDecorator struct {
	...
	EVMAssociatePriority int64
}

func NewEVMPreprocessDecorator(evmKeeper *evmkeeper.Keeper, accountKeeper *accountkeeper.AccountKeeper, EVMAssociatePriority int64) *EVMPreprocessDecorator {
	return &EVMPreprocessDecorator{evmKeeper: evmKeeper, accountKeeper: accountKeeper, EVMAssociatePriority: EVMAssociatePriority}
}
...
func (p *EVMPreprocessDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	if isAssociateTx && isAssociated {
		...
		return ctx.WithPriority(p.EVMAssociatePriority), nil // short-circuit without calling next
